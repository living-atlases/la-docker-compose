...
describe("Biocache offline downloads", () => {
  ...
  const DOWNLOAD_TIMEOUT_MS = Number(
    Cypress.env("DOWNLOAD_TIMEOUT_MS") || 240000
  );
  ...
  context(
    "a) API download with an email parameter (getDownloadUser route 3/4)",
    () => {
      before(function () {
        if (!downloadEmail) {
      ...
      it("refuses a download with no email and no principal (400 is CORRECT here)", () => {
        ...
        const url = serviceUrl(
          "recordsWs",
          `/occurrences/offline/download?${downloadQuery()}`
        );
        cy.request({ url, failOnStatusCode: false }).then((resp) => {
          expect(
            resp.status,
  ...
  context(
    "b) authenticated download from the hub (getDownloadUser route 1: JWT/OIDC)",
    () => {
      ...
      it("a logged-in user's download request is accepted by biocache-service", () => {
        ...
        cy.intercept("GET", "**/occurrences/offline/download*").as(
          "offlineDownload"
        );
        // failOnStatusCode:false because this test judges the DOWNLOAD, not the hub's health.
        // The hub currently 500s on the search page (already covered, and already failing, in
        // 2-biocache/search.cy.ts) — letting that abort here would hide the download verdict
        // behind an unrelated bug. A broken page simply yields no trigger, and we fall through
        // to the direct request below.
        ...
        cy.get("body").then(($body) => {
          ...
          cy.wait("@offlineDownload", { timeout: 60000 }).then(
            (interception) => {
  ...
  context("c) the queued download completes and the ZIP is fetchable", () => {
    ...
    // Self-contained on purpose: it issues its OWN download rather than reusing a statusUrl
    // from test (a). Cypress.env() looked like a cheap channel between tests, but the runner
    // is re-injected when the AUT changes origin (the login in (b) moves it from the portal
    // root to the records host), which drops anything written at runtime — so (c) reported
    // "no statusUrl from the API download test" in build #308 even though (a) had passed.
    // An extra queued download costs seconds; a test that lies about why it failed costs more.
    it("polls the statusUrl to a terminal state and downloads the archive", () => {
      cy.request({
        ...
        .then((resp) =>
          assertDownloadAccepted(
            resp,
            "download queued for the completion check"
          )
        )
        .then((queued) =>
          pollDownloadStatus(
            queued.statusUrl as string,
            Date.now() + DOWNLOAD_TIMEOUT_MS
          )
        )
        .then((status) => {
          // Case-insensitive: biocache-service reports the DownloadStatus enum lowercased
          // ("finished"), while the terminal-state check in pollDownloadStatus upper-cases
          // before comparing. Normalise here too rather than pinning one spelling.
          expect(
          ...
          expect(
            status.downloadUrl,
            "downloadUrl on the finished download"
          ).to.match(/^https?:\/\//);
          ...
          cy.request({
            ...
            if (zip.status === 404) {
              ...
              const alt = serviceUrl("recordsWs") + new URL(announced).pathname;
              cy.request({
                url: alt,
                encoding: "binary",
                failOnStatusCode: false,
      ...
      cy.wrap(null, { timeout: 60000 }).then(() =>
        mailhogMessagesTo(downloadEmail as string).then((messages) => {
          ...
          const subjects = messages.map((m) =>
            (m.Content?.Headers?.Subject || []).join(" ")
          );
          expect(
